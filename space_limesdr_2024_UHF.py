...
class space_limesdr_2024_UHF(gr.top_block, Qt.QWidget):

    def __init__(self, baud_rate=1200, equ_gain=0.01, excess_bw=0.35, fc_rx=436.83e6, fc_tx=145.91e6, loop_bw=0.0628, rx_gain_default=56, sps=160, tx_gain=56):
        ...
        self.settings = Qt.QSettings("GNU Radio", "space_limesdr_2024_UHF")
        ...
        self.limesdr_source_0.set_tcxo_dac(180)
        ...
        self.limesdr_sink_0.set_tcxo_dac(180)
        self.hdlc_framer_with_preamble_0 = hdlc_framer_with_preamble(
            num_postamble_bytes=10,
            num_preamble_bytes=20,
        )
        self.freq_xlating_fir_filter_xxx_0 = filter.freq_xlating_fir_filter_ccc(sps, firdes.low_pass(1.0,samp_rate,6000,100), fc_rx, samp_rate)
        self._fc_tx_1_range = Range(436.80e6, 436.86e6, 100, 436.83e6, 200)
        self._fc_tx_1_win = RangeWidget(self._fc_tx_1_range, self.set_fc_tx_1, 'fc_tx_1', "counter_slider", float)
        self.top_layout.addWidget(self._fc_tx_1_win)
    ...
    def set_baud_rate(self, baud_rate):
        ...
        self.root_raised_cosine_filter_1.set_taps(firdes.root_raised_cosine(1, self.samp_rate, 2*self.baud_rate, 0.0035, 160))
    ...
    def set_fc_rx(self, fc_rx):
        self.fc_rx = fc_rx
        self.freq_xlating_fir_filter_xxx_0.set_center_freq(self.fc_rx)
        self.limesdr_source_0.set_center_freq(self.fc_rx, 0)
        self.qtgui_freq_sink_x_0_0.set_frequency_range(self.fc_rx, 10000)
        self.qtgui_waterfall_sink_x_0.set_frequency_range(self.fc_rx, self.samp_rate)
    ...
    def set_samp_rate(self, samp_rate):
        ...
        self.set_rrc_taps(firdes.root_raised_cosine(1.0,self.samp_rate,2*self.samp_rate/self.sps,self.excess_bw,11*self.sps))
        self.freq_xlating_fir_filter_xxx_0.set_taps(firdes.low_pass(1.0,self.samp_rate,6000,100))
    ...
    def get_fc_tx_1(self):
        return self.fc_tx_1

    def set_fc_tx_1(self, fc_tx_1):
        self.fc_tx_1 = fc_tx_1
...
def argument_parser():
    ...
    parser.add_argument(
        "--tx-gain", dest="tx_gain", type=intx, default=56,
        help="Set tx_gain [default=%(default)r]")
    return parser


def main(top_block_cls=space_limesdr_2024_UHF, options=None):
    if options is None:
    ...
    tb = top_block_cls(baud_rate=options.baud_rate, fc_rx=options.fc_rx, fc_tx=options.fc_tx, sps=options.sps, tx_gain=options.tx_gain)
